...
func TestMsgCommand(t *testing.T) {
	...
	user1 := &model.User{TeamId: team.Id, Email: model.NewId() + "corey+test@test.com", Nickname: "Corey Hulen", Username: "user1", Password: "pwd"}
	user1 = Client.Must(Client.CreateUser(user1, "")).Data.(*model.User)
	store.Must(Srv.Store.User().VerifyEmail(user1.Id))

	Client.LoginByEmail(team.Name, user1.Email, "pwd")

	user2 := &model.User{TeamId: team.Id, Email: model.NewId() + "corey2+test@test.com", Nickname: "Corey Hulen 2", Username: "user2", Password: "pwd"}
	user2 = Client.Must(Client.CreateUser(user2, "")).Data.(*model.User)
	store.Must(Srv.Store.User().VerifyEmail(user2.Id))

	rs1 := Client.Must(Client.Command("", "/msg", true)).Data.(*model.Command)
	if rs1.Suggestions[0].Suggestion != "/msg user2" {
		t.Fatal("should have command to message other users")
	}

	user3 := &model.User{TeamId: team.Id, Email: model.NewId() + "corey3+test@test.com", Nickname: "Corey Hulen 3", Username: "user3", Password: "pwd"}
	user3 = Client.Must(Client.CreateUser(user3, "")).Data.(*model.User)
	store.Must(Srv.Store.User().VerifyEmail(user3.Id))

	rs2 := Client.Must(Client.Command("", "/msg user2", false)).Data.(*model.Command)
	if !strings.HasSuffix(rs2.GotoLocation, "/"+team.Name+"/channels/"+user1.Id+"__"+user2.Id) && !strings.HasSuffix(rs2.GotoLocation, "/"+team.Name+"/channels/"+user2.Id+"__"+user1.Id) {
		t.Fatal("failed to create direct channel")
	}

	rs3 := Client.Must(Client.Command("", "/msg user3 foobar", false)).Data.(*model.Command)
	if !strings.HasSuffix(rs3.GotoLocation, "/"+team.Name+"/channels/"+user1.Id+"__"+user3.Id) && !strings.HasSuffix(rs3.GotoLocation, "/"+team.Name+"/channels/"+user3.Id+"__"+user1.Id) {
	...
	rs4 := Client.Must(Client.Command("", "/msg user2", false)).Data.(*model.Command)
	if !strings.HasSuffix(rs4.GotoLocation, "/"+team.Name+"/channels/"+user1.Id+"__"+user2.Id) && !strings.HasSuffix(rs2.GotoLocation, "/"+team.Name+"/channels/"+user2.Id+"__"+user1.Id) {
